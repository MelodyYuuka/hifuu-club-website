...
(function () {
    ...
    Runner.prototype = {
        ...
        gameOver: function () {
            ...
            this.horizon.nightMode.draw();

            // 重绘云朵
            for (var i = 0; i < this.horizon.clouds.length; i++) {
                this.horizon.clouds[i].draw();
            }

            // 重绘障碍物
            for (var i = 0; i < this.horizon.obstacles.length; i++) {
                if (this.horizon.obstacles[i].isVisible()) {
                    this.horizon.obstacles[i].draw();
                }
            }
...
function updateStartText() {
    ...
    const h1Element = messageBox.querySelector("h1");

    if (isTouchDevice) {
        h1Element.textContent = "点击开始🐝";
    } else {
        h1Element.textContent = "按空格开始🐝";
    }
}
